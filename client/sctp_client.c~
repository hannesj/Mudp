...
int main(int argc, char **argv)
{
    ...
    if (mudpc_connect (sock, client_ip, argv[3+i], 9900) < 0)
    {
        ...
        const u_char *p)
{
   ...
   ip_hdr = (struct iphdr *)(p+ETHERNET_HEADER_LEN);
    ...
    memcpy (pkt, (p+ETHERNET_HEADER_LEN), header->caplen-ETHERNET_HEADER_LEN);

    /* Send the pack over MUDP */
    mudpc_send(sock, pkt, header->caplen-ETHERNET_HEADER_LEN);
